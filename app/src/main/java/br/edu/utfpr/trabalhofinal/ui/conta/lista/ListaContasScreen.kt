...
import br.edu.utfpr.trabalhofinal.utils.colorTextByType
import br.edu.utfpr.trabalhofinal.utils.colorTextByValueProjecao
import br.edu.utfpr.trabalhofinal.utils.colorTextByValueSaldo
import br.edu.utfpr.trabalhofinal.utils.formatar
import br.edu.utfpr.trabalhofinal.utils.valorFormatado
...
private fun List(
    ...
    LazyColumn(modifier = modifier) {
        items(contas) { conta ->
            ContaItem(conta, onContaPressed)
        }
    }
}
...
@Composable
private fun ContaItem(conta: Conta, onContaPressed: (Conta) -> Unit) {

    val color = conta.colorTextByType()
        ...
        headlineContent = {
            Row(
                ...
                Row(
                    ...
                    Column {
                        Text(conta.descricao, style = MaterialTheme.typography.bodyMedium)
                        Text(
                            conta.data.formatar(),
...
private fun BottomBar(
    ...
    Column(
        ...
        Totalizador(
            ...
            textColor = contas.colorTextByValueSaldo()
        )
        Totalizador(
            modifier = Modifier.padding(bottom = 20.dp),
            titulo = stringResource(R.string.previsao),
            valor = contas.calcularProjecao(),
            textColor = contas.colorTextByValueProjecao()
        )
    }
}
...
private fun gerarContas(): List<Conta> = listOf(
    ...
    Conta(
        descricao = "A Receber Vaga Alugada",
